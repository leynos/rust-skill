...
catalogue reachable only by an explicit invocation. That file is not part
of the Agent Skills manifest, so `skills-ref` does not see it and the
`make lint` targets cannot cover it.
